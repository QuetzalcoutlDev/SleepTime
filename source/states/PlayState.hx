...
class PlayState extends MusicBeatState
{
	...
	public static var allowBoomSpeed:Bool = false;
	public static var boomSpeed:Float;
	public static var bam:Float;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'catnapStage': new states.stages.CatnapStage(); //Week Catnap
		}
		...
		if(curStage == "catnapStage"){
			if(!ClientPrefs.data.lowQuality) {
				var vignette:BGSprite = new BGSprite("vignette", -660, -450, 1.0, 1.0);
				vignette.updateHitbox();
				add(vignette);

				var catnapHumoRojo:BGSprite = new BGSprite("humoRojo", -400, -550, 1.0, 1.0);
				catnapHumoRojo.updateHitbox();
				add(catnapHumoRojo);
			}
		}
		...
		if(curSong.toLowerCase() == "sleep-time" && ClientPrefs.data.shaders){ 
	...
	override function beatHit()
	{
		...
		if(allowBoomSpeed){
			if(curBeat % boomSpeed == 0 && !endingSong){
				triggerEvent("Add Camera Zoom", Std.string(0.015*bam), Std.string(0.03*bam), 0.0);
